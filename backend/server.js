...
// Import Models for Socket.IO logic
...
// Middleware to make the `io` object available in all API routes
...
// Specific GET routes for nested data that couldn't be easily handled by Express Router nesting
...
const socketToUserMap = {}; // Maps socket ID to user ID

io.on('connection', (socket) => {
  console.log('a user connected:', socket.id);
  
  // --- User Registration for Meetings ---
  ...
  socket.on('webrtc-offer', ({ sdp, offerTo }) => {
    socket.to(offerTo).emit('webrtc-offer', { sdp, offerFrom: socket.id });
  });
  socket.on('webrtc-answer', ({ sdp, answerTo }) => {
    socket.to(answerTo).emit('webrtc-answer', { sdp, answerFrom: socket.id });
  });
  socket.on('webrtc-ice-candidate', ({ candidate, sendTo }) => {
    socket.to(sendTo).emit('webrtc-ice-candidate', { candidate, sentFrom: socket.id });
  });
  ...
  socket.on('disconnect', () => {
    ...
    // You could also emit a 'user-left' event to the meeting rooms here
  });
});
